  ...
  describe('/api/users', function () {
    describe('POST', function () {
      ...
      it('Should reject users with non-string username', function() {
        ...
        return chai.request(app).post('/api/users').send(testUser)
          .then(res => {
            ...
            expect(res.body).to.be.a('object');
            expect(res).to.have.status(422);
            expect(res.body.message).to.equal('The \'username\' must be a string');
          });
      });
      ...
      it('Should reject users with non-string password', function() {
        ...
        return chai.request(app).post('/api/users').send(testUser)
          .then(res => {
            ...
            expect(res.body).to.be.a('object');
            expect(res).to.have.status(422);
            expect(res.body.message).to.equal('The \'password\' must be a string');
          });
      });
      ...
      it('Should reject users with password greater than 72 characters', function() {
        const password = 'passwordffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff';
        const testUser = {username, password, fullname};
        return chai.request(app).post('/api/users').send(testUser)
          .then(res => {
            expect(res).to.be.json;
      ...
      it('Should trim fullname', function() {
        ...
        return chai.request(app).post('/api/users').send(testUser)
          .then(res => {
            ...
            expect(res).to.have.status(201);
            expect(res.body).to.be.a('object');
            expect(res.body.fullname).to.equal('SonyaSlegers');
          });
      });
    });
  });
